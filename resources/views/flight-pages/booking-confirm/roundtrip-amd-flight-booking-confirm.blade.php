...
<body style="background-color:#fff !important;margin-top:100px;">
    <div class="container p-0">
        <div class="card-body">
            ...
            {{--<a class="btn float-right btn-sm"
                    href="{{ URL::to('/booking-pdf/flight-booking/' . $bookings->id * 2023).'/4' }}">
                    <i class="fa fa-download" aria-hidden="true">&nbsp;Download</i>
            </a>--}}
                <a class="btn float-right btn-sm" onclick="downloadPDF()">
                    <i class="fa fa-download" aria-hidden="true">&nbsp;Download</i>
                </a>
                
            <a class="btn float-right btn-sm"
                href="{{ URL::to('/booking-pdf/flight-booking/' . $bookings->id * 2022) }}">
                <i class="fa fa-download" aria-hidden="true">&nbsp;Print</i>
            </a>
                
             <div class="download-button">
                <button id="download">print</button>
                <p class="h3 text-success">Please save this ticket frist.</p>
            </div>
        </div>
        <div id="main-content" style="padding: 0px 53px;">
        <br>
        <h4 class="m-0 text-right text-dark font-weight-bold">Toll Free No.- +91 8069145571</h4>
                ...
                <p>
                    @foreach (json_decode($bookings->itinerary) as $itinerary)
                        @foreach (json_decode($bookings->passenger) as $passenger)
                            <small><strong>Booking Date:-{{ date('d-m-Y', strtotime($bookings->created_at)) }}
                                </strong> | <strong>Booking
                                    Time:- {{ date('H:i', strtotime($passenger->IssueDate)) }}</strong> </small>
                        @endforeach
                    @endforeach
                </p>
            </div>
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p>
                        @foreach (json_decode($bookings->itinerary) as $itinerary)
                            <small><strong>Booking Date:-
                                </strong>{{ date('d-m-Y', strtotime($bookings->created_at)) }} | <strong>Booking
                                    @foreach (json_decode($bookings->passenger) as $passenger)
                                        Time:-
                                </strong> {{ date('H:i', strtotime($passenger->IssueDate)) }}
                        @endforeach
                        </small>
                        @endforeach
                    </p>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="border-right text-center">
                                ...
                                <h6 class="small m-0">{{ date('d-m-Y', strtotime($itinerary->DepartDateTime)) }}
                                    |
                                    <strong>{{ date('H:i', strtotime($itinerary->DepartDateTime)) }}
                                </h6>
                            ...
                            <td class="border-right text-center">
                                ...
                                <h6 class="small m-0">
                                    <strong>{{ date('d-m-Y', strtotime($itinerary->ArrivalDateTime)) }} |
